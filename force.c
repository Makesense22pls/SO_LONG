...
/*   Updated: 2024/06/14 03:48:20 by mafourni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_printf("check [%d]\n", all->check);
	ft_printf("security [%d]\n", all->balade.security);
	if (all->check == 1 && all->balade.security == 0)
	{
		all->balade.security++;
		printf("check = [%d]", all->check);
		// printf("count = [%d]", all->count);
		all->balade.count ++;
		ft_final_sound(all);
		if (mlx_image_to_window(all->mlx, all->img_final_exit, all->pos_x_exit * 64, (all->pos_y_exit) * 64) < 0)
			return;
	}
}
...
void	chicken_sound_new_floor_and_player(t_all_the_time *all, size_t position_x_player, size_t position_y_player)
{
	...
	else
	{
		...
		if (mlx_image_to_window(all->mlx, all->img_floor, position_x_player * 64, position_y_player * 64) < 0)
			return;
		if (mlx_image_to_window(all->mlx, all->img_perso, position_x_player * 64, position_y_player * 64) < 0)
			return;
	}
}
